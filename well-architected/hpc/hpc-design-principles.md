...
| Configure [Azure CycleCloud autoscaling](/azure/cyclecloud/how-to/configure-autoscaling.md?view=cyclecloud-8#auto-scaling) to provision compute nodes based on scheduler queue depth and deallocate idle nodes. | Aligns compute capacity with demand and eliminates idle spend. |
...
| Deploy Azure CycleCloud with native support for [Slurm](/azure/cyclecloud/how-to/qs-deploy-ccws.md?view=cyclecloud-8#how-to-deploy), PBS Pro, Grid Engine, or other schedulers that align with existing workflows. | Preserves user productivity by allowing existing job scripts and submission models to run unchanged. |
