...
**Using governance policies**: Use [Azure Policy](/azure/governance/policy/overview) to define and enforce governance policies that align with your cost optimization goals. You can use Azure Policy to set rules on management groups, subscriptions, and resource groups. These policies can regulate resource provisioning, usage limits, and cost allocation. Use policies to promote rightsizing of resources, identify and eliminate idle or underutilized resources, and encourage the use of cost-effective services and architectures. Azure has several built-in policies for common use cases that provide a standardized and efficient way to enforce rules and guidelines across your Azure resources. We recommend that customers define their own custom policies to meet their unique business requirements or specific industry regulations not covered by Azure built-in policies. For more information about built-in policies, see [Azure Policy built-in policy definitions](/azure/governance/policy/samples/built-in-policies).
