...
| Use consumption-based pricing when it's practical.| You'll pay for what you use. This option might be more expensive than a fully utilized prepaid option. However, if you don't expect to fully utilize pre-purchased compute, consumption billing might be a better choice.|
| Switch to fixed-price billing instead of consumption-based billing for a resource when its utilization is high and predictable and a comparable SKU or billing option is available.| When utilization is high and predictable, the fixed-price model usually costs less and often supports more features.|
