...
If neither option is feasible, then you need to accept the extra cost. Rework the budgets as needed if the extra cost isn’t included.
...
**Usage analysis:** You can use Azure billing increments to analyze resource usage patterns and identify areas of high consumption. Understand how resources are utilized, so you can make informed decisions to optimize their usage and minimize unnecessary costs.

**Detailed cost analysis:** Microsoft Cost Management and Azure Advisor provide optimization recommendations that are based on usage and cost data, which helps you identify opportunities for cost savings. With this data, you can determine if resources are over-provisioned or underutilized, and right-size them to match the workload requirements. Right-sizing resources can help align to billing increments.
