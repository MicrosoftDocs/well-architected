...
You can use application groups in Azure Virtual Desktop (AVD) to manage and publish a set of applications or desktops to users. Application groups define which applications or desktops users can access and how they are presented. They help organize resources and control access efficiently, improving both security and user experience.
...
| Recommendation | Benefit |
|---|---|
| Define application groups based on specific user roles or departments (e.g., finance, HR, and IT) instead of a single, generalized group. | Enhances operational efficiency by providing users only with the applications they need. Reduces security risks by enforcing least-privilege access and minimizing unnecessary application exposure. |
...
| Enable Log Analytics and AVD Insights (AVDI) by onboarding your virtual machines using Azure Monitor Agent (AMA) to track user sessions, application performance, and connection health.| Improves proactive monitoring, enabling faster issue detection and troubleshooting.|
| Develop a criticality scale from 1 to 5 based on business requirements. For example: <br><br>  - *Level 5:* Mission-critical applications (e.g., financial systems) <br> - *Level 3:* Noncritical but important tools, such as document editing software <br> - *Level 1:* Optional or infrequently used applications | Helps prioritize resource allocation, maintenance, and high-availability configurations based on business impact.|
|Establish availability and reliability targets for application groups that support business-critical workloads.Define uptime expectations (e.g., 99.9% SLA) based on business requirements. While application groups in Azure Virtual Desktop (AVD) serve as logical containers for published applications and desktops, their availability and reliability depend on the resilience of the underlying infrastructure, including virtual machines, networking, profile storage, and authentication services. To ensure business continuity, infrastructure availability should be aligned with the criticality of each application group, implementing redundancy and failover mechanisms where necessary.| Sets clear performance expectations, enabling IT teams to proactively plan for resilience and service continuity. Ensuring high availability of session hosts, networking, and profile storage minimizes disruptions, providing users with seamless access to published applications and desktops.|
| Assign higher priority to business-critical application groups and host them on high-availability infrastructure. <br><br> For example, if you rate your finance department's application group a 5 (critical), assign the application group to host pools that have high-availability configurations and fault tolerance, such as availability zones or Azure Premium SSD storage. | Ensures high availability and minimizes downtime, improving business continuity and user experience.|
|Implement a BCDR (Business Continuity and Disaster Recovery) strategy with Azure Site Recovery and Azure Backup. Define RTO and RPO in Azure Backup policies.| Enhances the resilience of application groups by ensuring session hosts, user profiles, and application data can be quickly restored, minimizing disruptions and maintaining service continuity.|
...
| Use Azure Key Vault to securely store application secrets, such as API keys and database credentials, that the applications in application groups use. Ensure that you encrypt secrets to help protect them from unauthorized access. |Enhances security by centralizing secret management and encryption. Reduces the risk of credential exposure, simplifies credential rotation, and improves compliance with security policies. e.g. A financial application in an application group connects to an Azure SQL database and a third-party payment API. Without Azure Key Vault: Database credentials and API keys are stored in code or configuration files, increasing the risk of exposure and unauthorized access. |
