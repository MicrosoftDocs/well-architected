...
In Azure Virtual Desktop, do you know which areas Microsoft manages and which areas you're responsible for managing?
...
In an active-passive scenario, the host deployment is similar to the setup in an active-active scenario. For each host pool in the primary region, another host pool is deployed in the secondary region. But in an active-passive scenario, fewer, and up to 0, compute resources are active in the secondary region than in the primary region. The number of resources that are active usually depend on available budget. It's important to be aware that Azure capacity is not guaranteed in case of a regional outage where inactive resources must be turned on. Capacity is only guaranteed if the virtual machines are active. Consider on-demand Capacity Reservations to reserve Azure capacity for disaster recovery scenarios. For more information, see [On-demand Capacity Reservation](/azure/virtual-machines/capacity-reservation-overview).
...
If planning for a personal host pool, Site Recovery can be used to replicate session hosts from the primary region to the secondary region. This also requires having the proper infrastructure in place.
...
- If you use an active-active configuration, educate users about the proper use of resources.
...
User data should be redirected to a storage solution to minimize the amount of data in user profiles. By minimizing the data in user profiles, the need for protecting them is also minimized. A no-profile recovery strategy will lessen overhead but may affect the user experience in case of a disaster recovery scenario. 

If the Profile container content needs separate BCDR management and has different requirements compared to the Office container, they may be separated so they may be managed independently.
...
- An Azure region outage. If you have a multiregion deployment, you can use the FSLogix cloud cache feature to protect user profiles by replicating across regions.
...
- Use Azure Backup to back up FSLogix profiles that you store in Azure Files or Azure NetAppFiles.
...
When an entire Azure region experiences an outage, virtual networks are affected. Regional failures also affect the services that are deployed within virtual networks. To help protect your resources, you need to plan for a virtual network in your secondary region to allow for replication of personal host pool virtual machines or space for deployment of session hosts. You can also use Site Recovery to set up the virtual network in the failover region and preserve your primary network's settings. In a Virtual Desktop environment that's connected with your on-premises network, you should configure the virtual network in the secondary region with connectivity to the on-premises network.
