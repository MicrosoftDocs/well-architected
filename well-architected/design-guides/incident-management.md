...
1. <a name="mitigation-strategies"></a>Select an appropriate mitigation strategy. Choose mitigation approaches based on the current state of the workload, available resources, and immediate constraints. 
   ...
   The time it takes you to select the mitigation strategy is a component of your mean-time-to-recovery. When the onset of a user-facing report aligns with the rollout window of a change your team owns, the default response shouldn't be debating whether the new behavior is correct, but instead mitigation action. The cost of a rollback is one extra release cycle. The cost of delayed action is the entire remaining impact window for every user.
