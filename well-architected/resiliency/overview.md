...
- Commodity hardware helps keep cloud environments costs low, so occasional hardware failures can occur.
...
All of these factors mean that you should design cloud applications to expect occasional failures and recover from them. Azure has many resiliency features already built into the platform. For example:
...
- Azure managed disks are kept in different storage scale units to limit the effects of hardware failures.
...
|-----------------|-----------|
|[Reliability principles](principles.md)|Use these critical principles to assess the reliability of an application deployed on Azure.|
...
|[Availability Zones](/azure/architecture/high-availability/building-solutions-for-high-availability)|Use Availability Zones to spread a solution across multiple zones within a region. This set up allows for an application to continue functioning when one zone fails.|
