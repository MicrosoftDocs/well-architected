...
- Use Availability Sets with two or more VMs for workloads that cannot scale horizontally to protect against network, disk and power failures by distributing them across up to three fault domains and twenty update domains. See [availability set overview ](/azure/virtual-machines/availability-set-overview) and (Availability options for Azure Virtual Machines)[/azure/virtual-machines/windows/manage-availability] for more details.
