...
|---|---|
|**Active/Active design**|To maximise availability and achieve regional fault tolerance, solution components should be distributed across multiple Azure regions using an active/active deployment model where possible.|
...
|**Complexity avoidance**|Avoid unnecessary complexity when designing the solution and all operational processes to drive reliability and management efficiencies, minimising the likelihood of failures.|
...
|**Automation for hyperscale**|To minimise the impact to both performance and availability associated with unexpected or expected increases in traffic, scale operations throughout the solution should be fully automated, ensuring the time it takes to perform scale operations is understood and aligned with a model for application health through continuous validation efforts.
