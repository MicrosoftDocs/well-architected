...
The OMAT tool provides optimal right-sizing recommendations for Oracle on Azure IaaS that you can use to migrate your workload on Azure. Choose a hyper-threaded VM to save on licensing costs.

### Recommendations

- The [E-series](/azure/virtual-machines/edv5-edsv5-series) and [M-series](/azure/virtual-machines/m-series) VMs are the common choices for your database. Use the E-series for high throughput values and the M-series for workloads that require high memory. Both VMs are hyper-threaded enabled.
...
Other storage considerations are related to archived Oracle redo log files and backups. Archiving redo logs is typically an ongoing read/write operation. These logs need to be evaluated based on solution requirements and availability.
