...
|**Workload architecture:** Use the [Horizontal Pod Autoscaler](/azure/aks/concepts-scale.md#horizontal-pod-autoscaler).|Adjust the number of pods in a deployment depending on CPU utilization or other select metrics, which support cluster scale-in operations.|
