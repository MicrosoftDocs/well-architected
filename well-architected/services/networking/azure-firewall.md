...
| Enable [Azure Firewall (DNS) proxy](/azure/firewall/dns-details) configuration.| Enable this feature point clients in the VNets to Azure Firewall as a DNS server.It will protect internal DNS infrastructure that will not directly accessed and exposed. Azure Firewall must be also configured to use custom DNS that will be used to forward DNS queries.|
