...
-   Fully test your retry strategy under as wide a set of circumstances as possible, especially when both the application and the target resources or services that it uses are under extreme load. To check behavior during testing, you can:
    ...
    -   Consider using [libraries](https://learn.microsoft.com/microsoft-edge/test-and-automation/test-and-automation) or network tooling to intercept and modify traffic that replicates unfavorable scenarios from your automated tests, either by adding extra roundtrip times [simulating a slow network connection](https://learn.microsoft.com/microsoft-edge/devtools-guide-chromium/network/#simulate-a-slower-network-connection) or by changing the response for HTTP-based APIs (like the HTTP status code, headers, body, or other factors such us [blocking requests](https://learn.microsoft.com/microsoft-edge/devtools-guide-chromium/network/#block-requests)). Doing so enables deterministic testing of a subset of the failure conditions, for transient faults and other types of failures.
