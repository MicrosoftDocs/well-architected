...
- Virutal Network Flow logs. Review [flow logs](/azure/network-watcher/vnet-flow-logs-overview) and diagnostic logs.
...
- Use [Traffic Analytics](/azure/network-watcher/traffic-analytics) to analyse and enrich VNet flow logs to surface insights into the traffic flow. With built-in queries, you can obtain insights into blocked traffic, malicious flows, and active ports exposed to the internet across your Azure infrastructure. Use Traffic Analytics to support Zero Trust security by enabling segmentation, visibility into traffic flows, and detection of anomalous or unauthorised activity across your Azure environment.  
