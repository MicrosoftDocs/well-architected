...
Security measures should be integrated at multiple points into your existing software development lifecycle to (SDLC) to make sure: 

- Design choices do not lead to security gaps.
- Application code and configuration does not produce vulnerabilities through exploitable implementation and improper coding practices. 
- Acquired software added as part of supply chain do not introduce security threats.
- Application code, build, and deployment processes aren’t tampered.
- Vulnerabilities revealed through incidents are mitigated.
- Unused assets used are properly decommissioned.
- Compliance requirements are not compromised or reduced. 
...
- Evaluate the platform-provided affordances. 
  ...
  To ensure that this is being adhered to by all contributors to the software, maintain a list of approved and/or unapproved frameworks, libraries, vendors, as well as versions of each,  When possible, place guardrails in the development pipelines to support this list. Automate the use of tools to scan dependencies for vulnerabilities, as much as possible. 
...
- Define test plans.
  ...
  For more information, see [Application threat analysis](design-threat-model.md).
...
- Utilize security test tools.
  ...
  As part of your secure coding practices, follow industry standards on secure practices. For reference, see the links under [Community resources](#). 
...
- Protect developer environments.
  ...
  Build agents are highly privileged and have access to the build server and the code. They must be protected with the same rigor as your workload components.  This means access to it build agents must be authenticated and authorized, they should be network segmented with firewall controls, they should be subject to vulnerability scanning, and so on. The choice of Microsoft-hosted build agents should be preferred over self-hosted build agents. There are benefits such as clean virtual machine for each run of a pipeline. 
