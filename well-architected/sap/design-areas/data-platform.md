...
**Define RPO and RTO.** Creating database reliability requires a plan to recover from data loss. A logical error on the SAP database, a large-scale disaster, or a system outage can cause data loss. Your recovery plan should identify how much data loss is acceptable and how quickly normal operations must be restored. The acceptable data loss is the recovery point objective (RPO). The maximum restoration time is the recovery time objective (RTO). Track actual versus target RPO and RTO of your SAP application.
