...
Bootstrapping refers to configuration updates to a resource that must be made after the resource is provisioned, but before it's available to the workload as part of the workload pool. Bootstrapping is often thought of in relation to virtual machines (VMs), but many other resources need to be set up as part of the deployment process including platform as a service (PaaS) app hosting technologies and container hosting technologies like Azure Kubernetes Services (AKS).

Your cloud platform might provide bootstrapping solutions for you, which you should use instead of developing your own where possible. For example, you can take advantage of VM Extensions in Azure that allow you to make certain predefined configuration changes during the deployment process. Or you can customize your configuration changes by injecting PowerShell scripts.
