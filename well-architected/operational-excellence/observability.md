...
| **Telemetry** | Data collected from system components, including logs, metrics, traces, and events. Telemetry provides the foundation for observability. |
...
Ensure all components of the system, including application, infrastructure, and platform, emit meaningful telemetry. Capture logs and metrics consistently across all layers.

Use platform-provided monitoring tools when possible. They require minimal configuration and provide deep insights.

Adjust logging levels to balance observability needs with cost. Verbose logging can be expensive, so tailor verbosity for production and development environments. Implement correlation IDs to trace transactions across components.

Capture application events in structured, machine-readable formats such as JSON, MessagePack, or Protobuf. This enables easy processing and integration.  
...
Define workload health indicators, KPIs, and performance metrics early. Structure telemetry collection to reflect these targets.

Use metrics to track trends and logs to detect anomalies. Design dashboards to be actionable, allowing teams to quickly identify issues and take corrective action.
...
Move telemetry out of local silos and consolidate it into a central repository, if that's mandated by the organization. However for mission-critical workloads, autonmous storage of data is recommended. Use consistent formats and collection methods so data is accessible for analysis, dashboards, alerting, and reporting. Avoid manual retrieval from components, as it adds overhead and inconsistencies.

Separate data by environment. This prevents non-critical environment data from complicating production monitoring. Prioritize data transfer based on importance. Less urgent data can be transferred in batches, while time-sensitive information should be sent immediately.

For multi-region solutions, collect and store data regionally first, then aggregate it centrally. Use data consolidation services to:
- Deduplicate data.
...
Design observability to link telemetry from multiple sources such as metrics, logs, and traces. This enables tracing operations across services and diagnosing issues spanning multiple components.

Use correlation IDs consistently to track transactions across tiers. Choose collection models based on scale:
- Pull models work well for small applications.
- Push models suit distributed systems.

Aggregate data from presentation, middle, and data tiers for full operational visibility. Correlate application-level and resource-level logs to improve troubleshooting.

## Tailor storage and retention for usage patterns and cost

Select storage solutions based on query needs, urgency, and access patterns. Plan retention for short-term detailed data and long-term trend analysis.
...
Archive older telemetry to lower-cost storage. Keep frequently accessed data in faster storage systems. Implement data protection with resource locks, soft delete, and role-based access control.
...
Use proven monitoring patterns and architectures rather than custom approaches. Ensure dashboards are meaningful and actionable. Parameterized dashboards allow analysts to explore underlying data.
...
Set alerts based on workload health, not arbitrary values. Alerts should be actionable and provide context.
