...
|[Gateway Offloading](/azure/architecture/patterns/gateway-offloading)|Offloads request processing to a gateway device before and after forwarding the request to a backend node. Injecting an offloading gateway into the request process allows you to manage the configuration and upkeep of the offloaded functionality at a single point  instead of addressing it across multiple nodes.|
