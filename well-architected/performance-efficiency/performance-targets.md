...
**Identify key metrics.** Based on your workload objectives, identify the key metrics that will help you measure progress towards those goals. Consider metrics related to availability, capacity, and response time.
...
- *Response time* is assessed by analyzing the response time of the application. A common target is to have 99% of login requests completed in less than 1 second.
...
- *Consider the user:* When setting performance targets, it is crucial to adopt a user-centric perspective. Recognizing the user as the ultimate judge of performance helps ensure that performance targets align with user expectations. This involves considering both organizational objectives and the distinct requirements of the user base. By integrating these two aspects, performance targets can be tailored to reflect the desired user experience and overall workload effectiveness. By defining performance objectives that take into account user expectations, organizations can strive to provide a high-quality user experience and meet the needs of their users effectively
...
By regularly engaging with users, gathering feedback, and adapting performance targets, organizations can enhance the performance of their workloads and deliver optimal user experiences. To ensure performance targets are aligned with user expectations, it is important to establish an ongoing dialogue with users. This includes gathering user feedback to gain insights into evolving preferences, requirements, and technological trends. Regular communication with users allows for continuous refinement and adaptation of performance targets based on their changing needs.

By incorporating a user-centric perspective into the performance optimization strategy, organizations can create a foundation for workloads that resonate with users. This approach not only ensures that the workload meets technical benchmarks but also establishes a dynamic feedback loop for continuous improvement based on user needs. By prioritizing user satisfaction and loyalty, organizations position their workloads for sustained success.
