...
**Consider business goals.** Consider your business goals and translate them into technical requirements. For example, assume your goal is to provide an API that can handle file uploads from 1 million users, with an average file size of one MB and a storage time of one second or less. You'll need disks that have a write speed of 277.7 MBps and a CPU/NIC/memory configuration that can support 277 concurrent writes.
