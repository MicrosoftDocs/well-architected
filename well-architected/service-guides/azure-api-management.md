...
| *(Service)* Enable [automatic scale-out](/azure/api-management/api-management-howto-autoscale) based on traffic demands.<br/><br/> In multiregion deployments, secondary regions do not have automatic scale-out or in capabilities. You need to implement a custom function or Logic App, triggered on Azure Monitor metric alerts, to handle adding or removing units in response to demand. | Ensures you have enough resources available to meet demand from your API clients. Without enough capacity to handle new requests, clients would experience failures when contacting your API. |
...
| *(API)* Always define backend using the [backends](/azure/api-management/backends) feature of API Management. Reference those backends by ID in policy using [set-backend-service](/azure/api-management/set-backend-service-policy). Load balanced backends should be defined as a [backend pool](/azure/api-management/backends#load-balanced-pool). | This layer of configuration abstraction makes backend connection changes apply to all APIs using that backend without need for updating individual policy code, reducing risk of misconfiguration or an API policy change is forgotten. This is especially helpful when multiple APIs use the same backend. |
