...
>
> - **Exclude workspaces from critical path dependencies:** Log Analytics workspaces serve as important components of your observability system but shouldn't be included in your [workload's critical path](/azure/well-architected/reliability/identify-flows). These workspaces collect and store operational data essential for monitoring and troubleshooting. However, the core functionality of your workload must remain independent of workspace availability. This architectural separation ensures that observability system outages don't cascade into workload runtime failures.
>
...
|:---|:---|
| To support high durability of workspace data, deploy Log Analytics workspaces into a [region](/azure/azure-monitor/logs/availability-zones#data-resilience---supported-regions) that supports data resilience. Data resilience is only possible by linking the workspace to a [dedicated cluster](/azure/azure-monitor/logs/logs-dedicated-clusters) in the same region. | A dedicated cluster lets you spread the associated workspaces across availability zones, which provide protection against datacenter outages. <br><br> Even if you don't collect enough data now to justify a dedicated cluster, this preemptive regional choice helps support future growth. |
...
>
>    If you aren't set up to audit your workspace, you might be putting your organization at risk of being in breach of compliance requirements.
>
> - **Implement robust network controls:** Helps secure network access to your workspace and your logs through network isolation and firewall functions. Insufficiently configured network controls increase the risk of unauthorized or malicious access.
>
...
>
> - **Protect log data at rest through encryption:** Segmentation alone won't completely protect confidentiality of your log data. If unauthorized raw access occurs, having the log data encrypted at rest helps prevent bad actors from using that data outside of your workspace.
>
...
| Configure [data retention and archiving](/azure/azure-monitor/logs/data-retention-archive). Consider your specific requirements for having data readily available for log queries. <br><br> Configure [archived logs](/azure/azure-monitor/logs/data-retention-archive) to retain data for up to seven years and still access it occasionally through [search jobs](/azure/azure-monitor/logs/search-jobs) or [restoring a set of data](/azure/azure-monitor/logs/restore) to the workspace. | Data retention and archiving configuration significantly reduces your cost for long-term data retention beyond the default period while maintaining access to historical data when needed. |
...
>
> - **Ensure that your workspaces are healthy, and that you're notified when problems occur:** Like any other component of your workload, your workspaces can encounter problems. These problems can consume valuable time and resources to diagnose and fix, and might leave your team unaware of the production workload's status. Proactive monitoring of workspaces and early problem mitigation enable operations teams to reduce time spent on troubleshooting and repairs.
>
...
>
> - **Separate your nonproduction and production workloads:** Production-specific workspaces mitigate any overhead that nonproduction systems might introduce. It reduces the overall footprint of your workspaces, which requires fewer resources to handle log data processing.
>
