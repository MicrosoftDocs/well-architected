...
| (VMs & Scale set) **Use [Azure VM Applications](/azure/virtual-machines/vm-applications.md)** to package and deploy applications | Future-proof your application packaging and deployment across azure and benefit from built-in reliability, security, versioning and failure handling. VM Applications automatically replicates application packages across zones & regions and handle deployment failures. |  
...
>   
> - **Use trusted applications and deploy securely** Publish applications as [Azure VM Applications](/azure/virtual-machines/vm-applications.md) in Azure Compute Gallery and set RBAC rules for read & write access. This ensures trusted applications are installed by trusted users. VM Applications also provides application package from within Azure instead of from internet ensuring higher security. 
>
...
| (VMs & Scale set)  **Publish and deploy applications using [Azure VM Applications](/azure/virtual-machines/vm-applications.md)**| Publish applications as [Azure VM Applications](/azure/virtual-machines/vm-applications.md) in Azure Compute Gallery and set RBAC rules for read & write of the resource. This ensures trusted applications are installed by trusted users. VM Applications also provides application package from within Azure instead of from internet ensuring higher security. | 
...
>
> - **Automate processes for deploying applications, running scripts, and configuring VMs.** You can automate processes by using Azure VM Applications & extensions. We recommend the following options:
>   - Use [Azure VM Applications](/azure/virtual-machines/vm-applications.md) in Azure Compute Gallery to package applications once, share using RBAC or across the organization, and deploy across Azure VM & Scale Sets. Azure VM Applications also provides customizing applicaiton install/update/delete operations, versioning, failure handling and automatic regional and zonal replications. This enables teams to build VM applications as microservices and simplify application management.
>     
...
>
> - **Have processes for installing automatic updates.** Consider using [Automatic VM guest patching](/azure/virtual-machines/automatic-vm-guest-patching) and [Automatic Extension Upgrade](/azure/virtual-machines/automatic-extension-upgrade) for a timely rollout of critical patches, security patches, and new versions. Use [Azure Update Manager](/azure/update-manager/overview) to manage OS updates for your Windows and Linux VMs in Azure.
>
...
| (VMs, Scale Set) **Publish & deploy applications using [Azure VM Applications](/azure/virtual-machines/vm-applications.md) from Azure Compute Gallery** |  Azure VM Applications helps to package applications once, share using RBAC or across the organization, and deploy across Azure VM & Scale Sets. Azure VM Applications also provides customizing application install/update/delete operations, versioning, failure handling and automatic regional and zonal replications. This enables teams to build VM applications as microservices and simplify application management. |
| (Scale set) **Keep your VMs up to date** by setting an **[upgrade policy](/azure/virtual-machine-scale-sets/virtual-machine-scale-sets-upgrade-policy) and enabling [Automatic Extension Upgrade](/azure/virtual-machines/automatic-extension-upgrade)**. We recommend rolling upgrades. However, if you need granular control, choose to upgrade manually. <br><br> For Flexible orchestration, you can use  **[Azure Update Manager](/azure/update-manager/overview)**. | Security is the primary reason for upgrades. Security assurances for the instances shouldn't decay over time. <br><br> Rolling upgrades are done in batches. Automatic extension upgrades honors rolling upgrade more on VMSS and following Azure Safe Deployment Practices (SDP). This approach ensures that all instances aren't down at the same time. |
| **[Install prebuilt software components as extensions](/azure/virtual-machines/extensions/overview)** as part of bootstrapping. <br><br> Azure supports many extensions that can be used to configure, monitor, secure, and provide utility applications for your VMs. <br><br> **[Enable automatic upgrades](/azure/virtual-machines/automatic-extension-upgrade)** on extensions. |Extensions can help simplify the software installation at scale without you having to manually install, configure, or upgrade it on each VM. |
...
>
> - **Factor in the performance profile of VMs, scale sets, VM applications and disk configuration in your capacity planning.** Each SKU has a different profile of memory and CPU and behaves differently depending on the type of workload. Conduct pilots and proofs of concept to understand performance behavior under the specific workload. Test scale up latency with application deployment & VM configurations to ensure traffic surges are appropriately handled.
>
...
| (VMs, scale set) **Use [Azure VM Applications](/azure/virtual-machines/vm-applications.md) and create multiple replicas per region** | Multiple application replicas per region helps to download application across multiple VMs parallelly improving create latency for scale up operations. |
...
- **Automatic Extension Upgrade is enabled.** Check that automatic extension upgrade is enabled on scale sets to ensure that your VM extensions stay updated with security patches and new versions.
...
- **Required [Azure VM Applications](/azure/virtual-machines/vm-applications.md) are installed else inject them.** Check if Azure VM Applications required by your organization are installed on the VM. Policy can also be used to inject VM Application across all VMs. 
