...
| (Scale set) **Enable Trusted Launch on existing Virtual Machine Scale Sets** through [in-place upgrade capabilities](/azure/virtual-machines/trusted-launch-existing-vmss). <br><br>Trusted Launch provides hardware-based security features including secure boot and virtual Trusted Platform Module (vTPM) for enhanced security posture without requiring infrastructure redeployment. |Enhanced security through hardware-based security capabilities that can be retrofitted to existing infrastructure without service disruption, enabling organizations to improve compliance posture while maintaining operational continuity.|
