...
>
> - **Always use the SDK to access your databases.** Use the Cosmos DB SDK in your clients to access Cosmos DB, and [tune client side error handling parameters](/dotnet/api/microsoft.azure.documents.client.connectionpolicy.retryoptions?view=azure-dotnet) for both reads and writes. The SDK has built-in transient fault handling to minimize custom logic. Where built-in handling isn't sufficient, the SDK supports your custom error handling implementations, such as handling [rate limiting errors](/azure/cosmos-db/performance-tips?tabs=trace-net-core#429). The SDK also allows you to capture rich [diagnostics logs](/azure/cosmos-db/best-practice-dotnet#capture-diagnostics) on the client to provide reliability observability.
>
