...
| Configure [event batching](/azure/event-grid/delivery-and-retry#output-batching) to deliver multiple events in single operations. Apply [event filters](/azure/event-grid/event-filtering) at the subscription level to process only relevant events. | Reduces per-operation costs through batched delivery and eliminates costs from processing irrelevant events. |
