...
> - **Workload architecture:** High Availability: from a conceptual perspective, consider an Azure Stack HCI cluster as a logical "availability zone" used to host and run workloads. For business or mission-critical use cases, deploy workloads across two or more separate Azure Stack HCI clusters (*zones*), using application high availability patterns and practices such as synchronous or a-synchronous data replication and external network load balancing.
