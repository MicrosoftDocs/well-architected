...
> - **Workload architecture high availability:** From a conceptual perspective, consider an Azure Stack HCI cluster a logical "*availability zone*" to host and run workloads. For business or mission-critical use cases, deploy workloads across two or more separate Azure Stack HCI clusters, using application high availability patterns and practices, such as synchronous or a-synchronous data replication and external network load balancers to manage / direct user requests across multiple HCI clusters, ideally deployed in separate datacenter locations.
