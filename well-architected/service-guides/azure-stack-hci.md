...
- To determine the cost of Azure Stack HCI hardware, the high-level process involves selecting your preferred hardware OEM partner based on any existing business and commercial requirements, use the [Azure Stack HCI Solutions Catalog](https://aka.ms/hci-catalog#catalog) to review the list of validated nodes, integrated systems and premier solutions available. It is important to factor in the workload profile, such as the size, number and performance requirements of the workload to enable the selected hardware partners to discuss, optimize and finalize on the hardware cost for the required Azure Stack HCI nodes.
