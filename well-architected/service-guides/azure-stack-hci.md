...
|--------|----|
|(cluster) Reserve the equivalent of **[one capacity disk worth of space per node](/azure-stack/hci/concepts/plan-volumes#reserve-capacity)**, (_up to four nodes_) within the S2D storage pool.| Reserving capacity, by leaving unallocated space in the storage pool, enables S2D to repair "in-place" when a physical disk fails, which improves data resiliency and performance in the event of a physical disk failure.|
|(cluster) Ensure all physical nodes have network access to the list of **[required outbound https endpoints](/azure-stack/hci/concepts/firewall-requirements)** for Azure Stack HCI and Azure Arc.| To be able to reliably manage, monitor and operate Azure Stack HCI clusters and/or workload resources, the required outbound network endpoints must be accessible. Temporary interruption will not impact the running status of the workload, but may effect manageability.|
