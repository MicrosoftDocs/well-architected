...
>
>   Evaluate which platform features your reliability targets require. Dedicated workload profiles offer compute isolation and larger resource limits. Premium ingress allocates dedicated proxy resources with configurable timeouts to prevent bottlenecks.
...
| Implement service discovery [resiliency policies](/azure/container-apps/service-discovery-resiliency), such as retries, timeouts, and circuit breakers, to proactively prevent, detect, and recover from service request failures. | Enhance the reliability of your container apps by ensuring smoother and more resilient inter-service communication. |
...
| Enable [premium ingress](/azure/container-apps/premium-ingress) for workloads that require high-throughput or advanced HTTP routing. Configure dedicated resources, custom timeouts, and scaling settings for the ingress proxy. | Prevent ingress bottlenecks under heavy load by ensuring the ingress proxy has sufficient resources. Configurable timeouts and header limits help avoid dropped connections and request failures during traffic spikes. |
...
> - **Use premium ingress for latency-sensitive workloads.** Configure [premium ingress](/azure/container-apps/premium-ingress) to allocate dedicated compute resources to the ingress proxy. Configure custom timeouts and scaling settings to prevent the ingress layer from throttling requests under high concurrency.
