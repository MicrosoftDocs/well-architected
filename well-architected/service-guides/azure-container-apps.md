...
| Use [private endpoints](/azure/container-apps/how-to-use-private-endpoint) to restrict inbound access to your Container Apps environment through a private IP address in your virtual network. Disable public network access when you enable private endpoints. | Ensure reliable, private-only connectivity that is immune to public internet disruptions. Private endpoints eliminate external routing dependencies and reduce the blast radius of internet-related outages. |
| Enable [premium ingress](/azure/container-apps/premium-ingress) for workloads that require high-throughput or advanced HTTP routing. Configure dedicated resources, custom time-outs, and scaling settings for the ingress proxy. | Prevent ingress bottlenecks under heavy load by ensuring the ingress proxy has sufficient resources. Configurable time-outs and header limits help avoid dropped connections and request failures during traffic spikes. |
| Use [NFS Azure Files volume mounts](/azure/container-apps/storage-mounts) for Linux-based stateful applications that require shared persistent storage with low latency. Use a Premium storage account and ensure the Container Apps environment and storage account share the same virtual network. | Provide higher I/O performance than SMB for Linux workloads. NFS mounts support shared access across replicas, which enables stateful applications to maintain data consistency across container instances. |
...
>
>   *Use private endpoints.* Configure [private endpoints](/azure/container-apps/how-to-use-private-endpoint) for your Container Apps environment to ensure that inbound traffic never traverses the public internet. Disable public network access at the environment level when you use private endpoints. This approach provides stronger network isolation than internal ingress mode alone.
>
...
> - **Isolate untrusted code execution.** For workloads that run user-submitted or AI-generated code, use [dynamic sessions](/azure/container-apps/sessions) to execute code in secure, ephemeral sandboxes. Dynamic sessions use Hyper-V isolation to provide strong kernel-level separation between sessions. Each session is automatically destroyed after use, which prevents data leakage between executions.
...
| Use [private endpoints](/azure/container-apps/how-to-use-private-endpoint) in workload profile environments to restrict inbound access to a private IP address within your virtual network. Disable public network access at the environment level. | Ensure that traffic never traverses the public internet, which provides stronger network isolation than virtual network injection alone. Reduce exposure to internet-based attacks and meet compliance requirements for private-only connectivity. |
| Use [dynamic sessions](/azure/container-apps/sessions) to run untrusted or AI-generated code in isolated, ephemeral sandboxes. Configure session pools with appropriate cooldown periods and network isolation settings. Assign the Azure Container Apps Session Executor role via RBAC to control access to session pool APIs. | Provide strong Hyper-V isolation for each code execution session, which prevents malicious code from affecting the host or other sessions. Automatic session cleanup eliminates the risk of data leakage between executions. |
...
>
>   *Optimize GPU workload costs.* For AI and machine learning workloads, use [serverless GPUs](/azure/container-apps/gpu-serverless-overview) with per-second billing and scale-to-zero capabilities to eliminate costs during idle periods. Select the appropriate [GPU type](/azure/container-apps/gpu-types) based on workload requirements. Use cost-effective T4 GPUs for inference and lightweight tasks. Reserve A100 GPUs for large model training and high-compute scenarios.
...
| Use [serverless GPUs](/azure/container-apps/gpu-serverless-overview) with scale-to-zero for AI and machine learning workloads that have variable or bursty demand. Select the appropriate [GPU type](/azure/container-apps/gpu-types), such as T4 for inference or A100 for training. | Eliminate GPU costs during idle periods with per-second billing and automatic scale-to-zero. Avoid over-provisioning by matching GPU type to workload intensity, which can significantly reduce compute expenses for AI workloads. |
| Evaluate the cost of [premium ingress](/azure/container-apps/premium-ingress) and [private endpoints](/azure/container-apps/how-to-use-private-endpoint) against your traffic patterns and security requirements. These features incur additional charges compared to default ingress. | Avoid unnecessary costs by using premium networking features only when workload traffic patterns or compliance requirements justify the additional expense. |
...
>
>   *Use remote builds.* Use [cloud-based container builds](/azure/container-apps/quickstart-code-to-cloud) in Azure Container Registry to build container images directly in the cloud. Remote builds remove the dependency on local Docker toolchains and simplify CI/CD pipeline configurations.
...
>
>   *Use the managed OpenTelemetry agent.* Enable the [OpenTelemetry agent](/azure/container-apps/opentelemetry-agents) to emit standardized traces, metrics, and logs without deploying and managing a custom collector. The managed agent supports multiple export destinations and reduces the operational burden of maintaining observability infrastructure.
...
| Enable the managed [OpenTelemetry agent](/azure/container-apps/opentelemetry-agents) for standardized distributed tracing, metrics, and log collection. | Reduce the operational overhead of deploying and maintaining a custom OpenTelemetry collector. Provide consistent observability data across all container apps without requiring application-level instrumentation changes. |
| For Java workloads, use managed [Eureka Server for Spring](/azure/container-apps/java-eureka-server) and [Config Server for Spring](/azure/container-apps/java-config-server) components instead of self-hosting service discovery and configuration infrastructure. Configure [highly available Eureka clusters](/azure/container-apps/java-eureka-server-highly-available) for production workloads. | Reduce operational complexity by using fully managed Java components with built-in lifecycle management. Eliminate the need to patch, scale, and monitor self-hosted Eureka and Config Server instances. |
...
> - **Select appropriate GPU workload profiles for compute-intensive workloads.** For AI inference, machine learning, and high-performance computing workloads, select [serverless GPUs](/azure/container-apps/gpu-serverless-overview) for bursty demand or [dedicated GPU workload profiles](/azure/container-apps/workload-profiles-overview) for sustained, latency-sensitive workloads. Choose the appropriate [GPU type](/azure/container-apps/gpu-types), such as T4 for inference or A100 for training, based on your performance requirements. Minimize cold start latency for serverless GPUs by using lean container images, artifact streaming, and configuring minimum replica counts.
> - **Use premium ingress for latency-sensitive workloads.** Configure [premium ingress](/azure/container-apps/premium-ingress) to allocate dedicated compute resources to the ingress proxy. Configure custom time-outs and scaling settings to prevent the ingress layer from throttling requests under high concurrency.
...
| Use [serverless GPUs](/azure/container-apps/gpu-serverless-overview) for AI inference and machine learning workloads that have bursty demand patterns. Use [dedicated GPU workload profiles](/azure/container-apps/workload-profiles-overview) for sustained, latency-sensitive GPU workloads. Select the [GPU type](/azure/container-apps/gpu-types) that matches your performance requirements. | Ensure that compute-intensive workloads have access to appropriate GPU acceleration. Serverless GPUs provide elastic scaling for variable demand, while dedicated profiles provide consistent, low-latency performance for sustained workloads. |
| Enable [premium ingress](/azure/container-apps/premium-ingress) and configure dedicated resources, custom time-outs, and scaling for the ingress proxy in workloads that are latency-sensitive or high-throughput. | Eliminate ingress proxy bottlenecks that can increase response latency under high concurrency. Dedicated ingress resources help ensure consistent request throughput and predictable latency. |
| Use [NFS Azure Files volume mounts](/azure/container-apps/storage-mounts) with Premium storage for I/O-intensive Linux workloads that require low-latency shared storage. | Provide higher I/O throughput and lower latency than SMB mounts for Linux-based containers, which improves performance for workloads that read or write data frequently. |
