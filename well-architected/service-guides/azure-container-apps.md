...
| **Implement liveness and readiness probes for your container apps:** Using liveness and readiness probes ensures that your container apps are running smoothly and are ready to handle traffic. | Liveness probes help detect and restart containers that are in a failed state, while readiness probes ensure that only healthy containers receive traffic. This improves the overall stability and reliability of your applications by allowing the system to automatically handle unhealthy instances. |
| **Leverage Azure Container Apps' built-in observability features:** Use built-in observability tools such as log streaming, container console, Azure Monitor metrics, and alerts to ensure proactive monitoring and efficient debugging. | This enhances the reliability and stability of your applications by allowing quick identification and resolution of issues. |
...
- **Ensure Volume Mounts:** In cases where persistent storage is required, Container Apps environments should be configured with volume mounts to ensure data persistence and integrity.  
...
- **Enable authentication on Container Apps:** Ensure authentication is enabled to prevent anonymous HTTP requests and authenticate requests with tokens before they reach the Container App.
...
- **Disable public network access:** Disable public network access to improve security by exposing the Container Apps environment through an internal load balancer, preventing internet access to all Container Apps within the environment.
- **Disable external network access:** Enforce internal-only ingress to ensure inbound communication for Container Apps is limited to callers within the Container Apps environment.
- **Enforce HTTPS:** Ensure Container Apps are only accessible over HTTPS to protect data in transit from network layer eavesdropping attacks.
- **Managed Identity should be enabled:** Require managed identity for Container Apps to securely authenticate to any resource that supports Azure AD authentication.
...
| Recommendation | Benefit |
|----------------|---------|
| **Use the Consumption plan for development, testing, and low-traffic applications.** Provides a cost-effective solution for applications with unpredictable or low traffic. However, it does not support egress through firewall (UDR) or NAT Gateway support. |
...
| **Deprovision the entire non-production environment during off-peak times and weekends using Infrastructure as Code (IaC) and re-provision when needed.** Significantly reduces costs by eliminating resource usage during periods of inactivity, ensuring you only pay for resources when they are actively being used. This approach optimizes resource utilization and cost efficiency. |
...
While there are no built-in policies that are related to cost optimization, custom policies can be created which can enforce cost optimization constraints.

- **Enforce tagging for cost management:** Require tags on all resources (e.g., environment, owner, cost-centre) to ensure detailed cost tracking and management.
...
- **Enable diagnostics settings:** Enforce the configuration of diagnostic settings to ensure logs and metrics are consistently collected and monitored, providing visibility into application performance and health.
...
- **Enable Diagnostics and Monitoring**: Enforce the configuration of diagnostic settings to ensure logs and metrics are consistently collected and monitored, providing visibility into application performance and health.
