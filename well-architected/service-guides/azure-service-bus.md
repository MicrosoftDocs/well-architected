...
>
> - **Implement disaster recovery and backup strategies:** Multi-region disaster recovery protects against complete regional failures through Service Bus geo-replication and geo-disaster recovery capabilities. For Premium tier namespaces, geo-replication is the recommended because it provides protection by replicating both metadata and message data across regions with automatic failover capabilities.
>
>    Geo-replication offers synchronous and asynchronous replication modes: synchronous provides zero data loss with higher latency, while asynchronous minimizes performance impact with potential data loss only during forced promotion scenarios. Geo-disaster recovery remains available as an alternative that replicates only namespace metadata without message data, suitable for scenarios where applications handle their own data replication.
>
>    During regional outages, applications can continue operations in the promoted secondary region. Recovery planning for geo-replication should account for replication lag monitoring, promotion procedures (planned vs. forced), and DNS propagation timing during failover scenarios.
>
...
| Configure [geo-replication](/azure/service-bus-messaging/service-bus-geo-replication) for Premium tier namespaces to enable disaster recovery with both metadata and message data replication across regions. Choose synchronous replication for zero data loss or asynchronous replication for optimized performance with minimal data loss risk.<br><br>Alternately you can configure [geo-disaster recovery pairing](/azure/service-bus-messaging/service-bus-geo-dr) for scenarios requiring only metadata replication where applications manage their own data recovery strategies. | Geo-replication helps with protection against regional failures through data and metadata replication, supporting planned and forced promotion scenarios with configurable consistency models.<br><br>Geo-disaster recovery offers lightweight metadata-only protection suitable for applications with custom data replication or acceptable message loss during disasters. |
...
>
>    Match Service Bus capabilities to actual environment requirements. Production needs zone redundancy, geo-replication or geo-disaster recovery, and dedicated throughput. Non-production environments can use simplified feature sets, lower throughput, and reduced availability guarantees to minimize costs.
>
...
<!-- Updated: 2026-02-10 for Azure Update 490149 -->