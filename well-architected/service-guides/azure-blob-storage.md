...
All data and metadata written prior to the last sync time is available on the secondary, while data and metadata written after the last sync time might not have been written to the secondary and might be lost.|
...
|If you configure a diagnostic setting that sends resource logs to a Log Analytics workspace, make sure to configure a retention period in that workspace. | Setting a retention period avoids extra charges related to the use of Log Analytics.|
|If you only plan to query logs occasionally (for example, query logs for compliance auditing), consider sending resource logs to a storage account instead of sending them to a Log Analytics workspace. To analyze logs, you could use a serverless query solution such as Azure Synapse. See [Optimize cost for infrequent queries](/azure/storage/blobs/blob-storage-monitoring-scenarios#optimize-cost-for-infrequent-queries). | The cost of using logs in Log Analytics is high. Storing resource logs in a storage account for later analysis can reduce cost.|
|If you choose to send resource logs to a storage account, use lifecycle management policies to delete or archive them. If you choose to archive logs, you'll have to convert them to block blobs first. See [Tier append and page blobs](/azure/storage/blobs/access-tiers-best-practices#tier-append-and-page-blobs). | You can't set a retention policy on resource logs that are sent to a storage account. This can lead to large numbers of logs files building up over time and lead to unnecessary capacity charges.|
...
|Set retention periods for soft delete. Consider starting with a short retention period to better understand how the feature affects your bill. The minimum recommended retention period is seven days. | A retention period keeps soft-deleted blobs from piling up and adding to the cost of capacity.|
...
|Disable any encryption scopes that aren't needed to avoid unnecessary charges. | DeEncryptions scopes incur a per month charge.|
...
|When uploading blobs or blocks, use a blob or block size that is greater than 256 KiB. | Blob or block sizes above 256 KiB uses the performance enhancements of high-throughput block blobs.|
