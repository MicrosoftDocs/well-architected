...
Azure Front Door is a global load balancer that's suitable for HTTP and HTTPS traffic and a Content Delivery Network (CDN). Azure Front Door delivers and distributes traffic that's closest to the application users. 
...
Start your design strategy based on the [design review checklist for Reliability](../reliability/checklist.md) and determine the checklist items' relevance to your business requirements. Keep the tiers and CDN capabilities in mind, and extend the strategy to include more approaches as needed.
...
>
> - **Choose your deployment strategy**. The fundamental approaches are *active-active* and *active-passive*.  Active-active deployment means that multiple environments or stamps that run the workload serve traffic. Active-passive deployment means that only the primary handles all traffic, but it fails over to the secondary when necessary. In multiregion deployment, stamps run in different regions for higher availability with a global load balancer, like Azure Front Door, that distributes traffic. Therefore, it's important for the load balancer to know which approach to take. 
> 
...
The Security pillar provides confidentiality, integrity, and availability guarantees for the workload.

The [Security design principles](/azure/well-architected/security/security-principles) provide a high-level design strategy to achieve those goals by applying approaches to the technical design that restrict the traffic coming through Azure Front Door.
...
>
> - **Monitor anomalous activity**. Regularly review the logs to check for attacks and false positives. Send [WAF logs from Azure Front Door](/azure/web-application-firewall/afds/waf-front-door-monitor) to the organization's centralized SIEM, such as Microsoft Sentinel, to detect threat patterns and incorporate preventative measures in the workload design. 
...
>
> - **Optimize incoming requests**. Azure Front Door bills the incoming requests. You can set restrictions in your design choices. 
>    
...
>
>   Use caching in Azure Front Door for content that doesn't change often. Because content is served from cache, you save on bandwidth costs that are incurred when the request is forwarded to the backends.
>
...
|Disable health checks in single backend pools.<br>If you have only one origin configured in your Azure Front Door origin group, these calls are unnecessary.|You can save on bandwidth costs for requests that aren't required to make routing decisions.|
...
Start your design strategy based on the [design review checklist for Operational Excellence](../operational-excellence/checklist.md). Define processes for observability, testing, and deployment that's related to Azure Front Door.
...
>   - Minimize the number of backend requests in the application. 
>
...
>
>      By aggregating requests, you send less data between the frontend and the backend and establish fewer connections between the client and the backend, which reduces overhead. Also, Azure Front Door handles fewer requests, which prevents overload.
> 
> - **Optimize the use of health probes**. Get health information from health probes only when the state of the origins have changed. Strike a balance between monitoring accuracy and minimizing unnecessary traffic.
>
...
|Recommendation|Benefit|
|------------------------------|-----------|
| [Enable caching](/azure/frontdoor/front-door-caching). <br><br> You can optimize [query strings for caching](/azure/frontdoor/front-door-caching). For purely static content, ignore query strings to maximize your use of the cache. <br><br> If your application uses query strings, consider including them in the cache key. Including the query strings in the cache key allows Azure Front Door to serve cached responses or other responses, based on your configuration.| Azure Front Door offers a robust CDN solution that caches content at the edge of the network. Caching reduces the load on the backend servers and reduces data movement across the network, which helps offload bandwidth usage.|
...
Azure provides an extensive set of built-in policies that are related to Azure Front Door and CDN. Some of the preceding recommendations can be audited and enforced through Azure Policy. For example, you can check if:
...
For comprehensive governance, review the [built-in definitions for CDN](/azure/governance/policy/samples/built-in-policies#cdn) and other Azure Front Door policies that are listed in [Azure Policy built-in policy definitions](/azure/governance/policy/samples/built-in-policies).
