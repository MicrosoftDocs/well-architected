...
|As part of your solution-wide availability and disaster recovery strategy, consider [enabling geo replication](/azure/event-hubs/geo-replication), if available for your tier. | This option allows asynchronous replication of metadata and messages in a secondary namespace in another region. The client should have idempotency when resuming processing in the secondary region from the last checkpoint.|
...
|The number of partitions reflects the degree of downstream parallelism you can achieve.|For maximum throughput, use the maximum number of partitions supported by the SKU when creating the Event Hub. Increasing the number of partitions enables you to scale concurrent processing entities to match the partitions, ensuring optimal send and receive availability.|
