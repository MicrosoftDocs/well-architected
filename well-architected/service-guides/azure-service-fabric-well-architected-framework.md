...
> - (Cluster and workload) Use the Service Fabric [health model](/azure/service-fabric/service-fabric-health-introduction) to continuously monitor the health of your solution. This tool should complement your overall [workload health model](../design-guides/health-modeling)
...
> [!div class="checklist"]
> - **TODO: _Remove the link (is already in the recommendations table) and generalize the text of the recommendation about PE:07#optimize-infrastructure-performance "https://learn.microsoft.com/en-us/azure/well-architected/performance-efficiency/optimize-code-infrastructure#optimize-infrastructure-performance". For example: enhancing and adjusting infrastructure elements to ensure peak operation and the best use of resources for a workload. Use optimization of VM configuration as an example_** (Cluster) [Exclude the Service Fabric processes from Windows Defender](/azure/service-fabric/service-fabric-best-practices-security#windows-defender) to improve performance.
> - **TODO:_Merge the next two into a single design checklist item that aludes to PE:02 Capacity Planning and PE:07#optimizing-infra-performance. Select the right SKU/tier and sizes to meet the needs identified during capacity planning_** (Cluster) Select appropriate VM SKU.
> - (Cluster and workload) Use appropriate managed disk tier and size.
> - **TODO: _Expand to explain a bit about the different programming models and what their benefits are. Something like: Understand the available programming models available for your Service Fabric workloads and base your design on the one that more closely fits your requirements. You can take advantage of built-in features that your workload may need. such as the need for state management, concurrency, reusing existing codebase, and deployment preferences ----- Remove the link from here as it will appear in the recommendations table_** (Workload)  Decide what [programming model](/azure/service-fabric/service-fabric-choose-framework) you will use for your services.
> - **TODO: NEW checklist item for the cloud pattern recommendations (microservices, event driven, and background processing). Details are in the comments in the recommendation table_**

### Recommendations

|Recommendation|Benefit|
|-----------------------------------|-----------|
| **TODO:_Clarify this is for Windows VMs and restructure recommendation vs benefit_** (Cluster) [Exclude the Service Fabric processes from Windows Defender](/azure/service-fabric/service-fabric-best-practices-security#windows-defender) if your security policies allow you to exclude processes and paths for open-source software.| By default, Windows Defender antivirus is installed on Windows Server 2016 and 2019. Excluding Service Fabric processes reduce the performance impact and resource consumption overhead incurred by Windows Defender|
| **TODO:_Rearrange to restructure recommendation vs benefit_**(Cluster) Consider using [Autoscaling](/azure/service-fabric/how-to-managed-cluster-autoscale) for your cluster. Enabling addition or reduction of nodes on demand on a secondary node type.| Autoscaling reduces the management overhead and potential business impact by monitoring and optimizing the amount of nodes servicing your workload.|
...
| **TODO: _Move these three recommendations (microservices, event driven, and background processing) to the design checklist. Integrate the content into a single NEW checklist, aligned with PE:07 optimizing code and infrastructure, and specifically around using cloud architecture patterns to design your workloads. Include the three examples provided_**  (Workload)  Leverage loosely-coupled [microservices](/azure/architecture/guide/architecture-styles/microservices) for your workloads where appropriate.|Using microservices allows you to get the most out of Service Fabric's features.|
...
| **TODO:_ Aligned with PE:05 Scaling. --- Replace the phrase "review..." with Implement or Follow recommendations. _** (Cluster and workload) Review the [different ways you can scale your solution in Service Fabric](/azure/service-fabric/service-fabric-concepts-scalability).|You can use scaling to enable maximum resource utilization for your solution.|
