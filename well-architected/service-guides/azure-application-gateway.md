...
| Recommendation | Benefit |
|--------|----|
| Set up a TLS policy for enhanced security |Set up a [TLS policy](azure/application-gateway/application-gateway-ssl-policy-overview#predefined-tls-policy) for extra security. Ensure you're using the latest TLS policy version (AppGwSslPolicy20220101S). This enforces TLS 1.2 and stronger ciphers. |
