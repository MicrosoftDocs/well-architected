...
| Recommendation | Benefit |
|--------|----|
| Enable [dedicated backend connections](/azure/application-gateway/configuration-http-settings?tabs=backendhttpsettings#dedicated-backend-connection) for applications with frequent, short-lived connections or heavy connection workloads. | Dedicated backend connections provide persistent connectivity between Application Gateway and backend services, improving performance for connection-intensive applications while maintaining load balancing benefits. This reduces connection establishment overhead and improves database connection pooling efficiency for high-throughput scenarios, though organizations should plan for connection scaling and resource management. |
...
You might have to make design tradeoffs when implementing the approaches in the pillar checklists. Here are some examples of advantages and drawbacks.

:::image type="icon" source="../_images/trade-off.svg"::: **Dedicated backend connections**

- **Enable dedicated connections:** Use dedicated backend connections for applications with heavy load and frequent, short-lived connections. This approach provides persistent connectivity between Application Gateway and backend services, reducing connection establishment overhead and improving database connection pooling efficiency for high-throughput scenarios.

  This strategy optimizes performance for connection-intensive applications while maintaining load balancing benefits. It's particularly effective for workloads that handle small packages with frequent connections, where connection setup time becomes a significant performance bottleneck.

  Also consider the disadvantages, like increased resource consumption on the gateway side. Dedicated connections require more memory and processing power on the Application Gateway instances to maintain persistent connections. Organizations need careful planning for connection scaling and resource management to avoid overprovisioning while ensuring optimal performance.

- **Use shared connections:** Traditional shared connection pooling minimizes resource usage on the Application Gateway and provides cost-effective load balancing for most standard workloads. This approach works well for applications with longer-lived connections or lower connection frequency.

  As a disadvantage, shared connections can introduce connection establishment overhead for high-frequency, short-lived connection patterns. Applications with heavy connection churn might experience higher latency due to repeated connection setup and teardown cycles.
